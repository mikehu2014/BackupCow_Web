...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    ...
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:BackupCowConnectionString %>"
        SelectCommand="SELECT * FROM [TrialAnalyze] WHERE ([Type] = @Type)" OldValuesParameterFormatString="original_{0}" ConflictDetection="CompareAllValues" DeleteCommand="DELETE FROM [TrialAnalyze] WHERE [ID] = @original_ID AND (([Year] = @original_Year) OR ([Year] IS NULL AND @original_Year IS NULL)) AND (([Type] = @original_Type) OR ([Type] IS NULL AND @original_Type IS NULL)) AND (([Number] = @original_Number) OR ([Number] IS NULL AND @original_Number IS NULL)) AND (([Value] = @original_Value) OR ([Value] IS NULL AND @original_Value IS NULL)) AND (([Memo] = @original_Memo) OR ([Memo] IS NULL AND @original_Memo IS NULL))" InsertCommand="INSERT INTO [TrialAnalyze] ([Year], [Type], [Number], [Value], [Memo]) VALUES (@Year, @Type, @Number, @Value, @Memo)" UpdateCommand="UPDATE [TrialAnalyze] SET [Memo] = @Memo WHERE [ID] = @original_ID">
        <SelectParameters>
